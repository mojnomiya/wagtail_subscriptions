...
class SubscriptionAnalytics:
    ...
    def get_churn_rate(days=30):
        ...
        churned = Subscription.objects.filter(
            canceled_at__gte=start_date, canceled_at__lt=end_date
        ).count()
    ...
    def get_feature_usage(feature_slug, days=30):
        ...
        # Validate feature exists to prevent injection
        try:
            feature = Feature.objects.get(slug=feature_slug)
        except Feature.DoesNotExist:
            return {"total_usage": 0, "avg_usage": 0, "unique_users": 0}

        end_date = timezone.now()
        start_date = end_date - timedelta(days=days)

        usage = UsageRecord.objects.filter(feature=feature, period_start__gte=start_date).aggregate(
