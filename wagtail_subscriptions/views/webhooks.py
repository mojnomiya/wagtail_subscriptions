...
class PaddleWebhookView(View):
    ...
    def post(self, request, *args, **kwargs):
        payload = request.body
        signature = request.POST.get("p_signature", "")

        try:
            processor = get_payment_processor("paddle")
            event_data = processor.process_webhook(payload, signature)

            # Store and process webhook event
            webhook_event, created = WebhookEvent.objects.get_or_create(
            ...
            self.process_paddle_event(event_data)
    ...
    def process_paddle_event(self, event_data):
        ...
        if event_type == "subscription_updated":
            self.handle_paddle_subscription_updated(event_data["data"])
        elif event_type == "subscription_cancelled":
            self.handle_paddle_subscription_cancelled(event_data["data"])

    def handle_paddle_subscription_updated(self, data):
        """Handle Paddle subscription updates"""
        try:
            subscription = Subscription.objects.get(external_id=data.get("subscription_id"))
            subscription.status = data.get("status", "active").lower()
            subscription.save()
        except Subscription.DoesNotExist:
            pass

    def handle_paddle_subscription_cancelled(self, data):
...
class PayPalWebhookView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            event_data = processor.process_webhook(payload, "")
    ...
    def process_paypal_event(self, event_data):
        ...
        if event_type == "BILLING.SUBSCRIPTION.ACTIVATED":
            self.handle_paypal_subscription_activated(event_data["data"])
        elif event_type == "BILLING.SUBSCRIPTION.CANCELLED":
            self.handle_paypal_subscription_cancelled(event_data["data"])

    def handle_paypal_subscription_activated(self, data):
        """Handle PayPal subscription activation"""
        try:
            subscription = Subscription.objects.get(external_id=data.get("id"))
            subscription.status = "active"
            subscription.save()
        except Subscription.DoesNotExist:
            pass

    def handle_paypal_subscription_cancelled(self, data):
