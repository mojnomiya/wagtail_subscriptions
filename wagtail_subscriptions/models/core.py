...
class SubscriptionPlan(models.Model):
    ...
    trial_period_days = models.PositiveIntegerField(
        default=0, verbose_name=_("Trial Period (days)")
    )
...
class Subscription(models.Model):
    ...
    def __str__(self):
        return f"{escape(str(self.user))} - {escape(self.plan.name)} ({escape(self.status)})"
    ...
    def has_feature_access(self, feature_slug):
        ...
        try:
            ...
            return True
        except (AttributeError, self.plan.plan_features.model.DoesNotExist):
            return False
...
class Customer(models.Model):
    ...
    def __str__(self):
        return escape(self.user.get_full_name() or self.user.username)
