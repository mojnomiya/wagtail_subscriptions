...
class SubscriptionRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not plan:
            messages.warning(
                request, _("An active subscription is required to access this feature.")
            )
...
class FeatureRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.required_feature and not TenantSubscriptionManager.has_feature_access(
            request, self.required_feature
        ):
            messages.warning(
                request,
