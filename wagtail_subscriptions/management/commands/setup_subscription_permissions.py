...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        subscription_managers, created = Group.objects.get_or_create(name="Subscription Managers")

        if created:
            self.stdout.write(self.style.SUCCESS('Created "Subscription Managers" group'))
        ...
        for feature_slug in common_features:
            try:
                ...
                self.stdout.write(self.style.WARNING(f'Feature "{feature_slug}" not found'))

        self.stdout.write(self.style.SUCCESS("Successfully set up subscription permissions"))
