# Wagtail Subscriptions Documentation

## 📚 Documentation Structure

### Getting Started
- [Installation Guide](installation.md) - How to install and configure
- [Quick Start](QUICK_START.md) - Get up and running quickly
- [README](../README.md) - Project overview and features

### Development
- [Contributing Guide](CONTRIBUTING.md) - How to contribute to the project
- [Test Guide](TEST_GUIDE.md) - Running and writing tests
- [Changelog](../CHANGELOG.md) - Version history and changes

### Sprint Documentation
- [Sprint 1: Security & Core Fixes](SPRINT_1_COMPLETE.md)
- [Sprint 2: Payment Processors](SPRINT_2_COMPLETE.md)
- [Sprint 3: Customer Portal & UI](SPRINT_3_COMPLETE.md)
- [Sprint 4: Admin Features](SPRINT_4_COMPLETE.md)
- [Sprint 5: REST API (Planned)](SPRINT_5_PLAN.md)

### Release Management
- [Pre-Release Checklist](PRE_RELEASE_CHECKLIST.md) - v1.0.0 preparation
- [Recommended Steps Complete](RECOMMENDED_STEPS_COMPLETE.md) - Recent improvements

## 🔗 External Resources
- [GitHub Repository](https://github.com/mojnomiya/wagtail_subscriptions)
- [Issue Tracker](https://github.com/mojnomiya/wagtail_subscriptions/issues)
- [PyPI Package](https://pypi.org/project/wagtail-subscriptions/)
